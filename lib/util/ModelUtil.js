...
const ARCHIMATE4_ASPECTS = {
  'active-structure': CX.ASPECT_ACTIVE,
  behavior: CX.ASPECT_BEHAVIOR,
  'passive-structure': CX.ASPECT_PASSIVE,
  motivation: CX.ASPECT_MOTIVATION,
  other: CX.ASPECT_ACTIVE,
};

function getPaletteClassName(concept) {
  ...
  return `archimate-${domain}-${suffix}`;
}

export const ARCHIMATE4_ELEMENT_MAP = new Map(
  Archimate4Registry.concepts.map((concept) => [ concept.id, {
    className: getPaletteClassName(concept),
    domain: concept.domain,
    layer: concept.domain,
    aspect: ARCHIMATE4_ASPECTS[concept.category],
    typeName: concept.displayName,
    pictoRef: concept.notation.rendererRef,
    notation: concept.notation,
    relationshipRuleSource: concept.relationships.ruleSource,
  } ])
);

export function getArchimate4Concept(elementType) {
  return Archimate4Registry.getConcept(elementType);
}
...
export function getLayerType(elementType) {
  const definition = elementType && ARCHIMATE4_ELEMENT_MAP.get(elementType);
  return definition ? definition.layer : elementType && FULL_ELEMENT_MAP.get(elementType) && FULL_ELEMENT_MAP.get(elementType).layer;
}

export function getAspectType(elementType) {
  const definition = elementType && ARCHIMATE4_ELEMENT_MAP.get(elementType);
  return definition ? definition.aspect : elementType && FULL_ELEMENT_MAP.get(elementType) && FULL_ELEMENT_MAP.get(elementType).aspect;
}

export function getTypeName(elementType) {
  const definition = elementType && ARCHIMATE4_ELEMENT_MAP.get(elementType);
  return definition ? definition.typeName : elementType && FULL_ELEMENT_MAP.has(elementType) && FULL_ELEMENT_MAP.get(elementType).typeName;
}
...
export function getPictoRef(elementType) {
  const definition = elementType && ARCHIMATE4_ELEMENT_MAP.get(elementType);
  return definition ? definition.pictoRef : elementType && FULL_ELEMENT_MAP.has(elementType) && FULL_ELEMENT_MAP.get(elementType).pictoRef;
}
