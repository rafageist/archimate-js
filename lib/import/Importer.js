...
export function displayGraphicalView(baseViewer, model, view) {
    ...
    if (view.viewElements) {
      ...
      for (const viewElement of view.viewElements) {
        if (is(viewElement, ARCHIMATE_CONNECTION)) {
          ...
          try {
            ...
            throw new Error(`Failed to import ${elementToString(viewElement)}: ${e.message}`);
          }
        }
      }
      ...
      for (const connectionElement of tempered) {
        try {
          ...
          throw new Error(`Failed to import ${elementToString(connectionElement)}: ${e.message}`);
        }
      }
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      if (eventBus) {
        eventBus.fire('import.render.complete', { error: e, warnings: warnings });
      }
      return reject(e);
    }
  });
