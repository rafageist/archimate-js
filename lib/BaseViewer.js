...
import { validateModelRelationships } from './archimate4/RelationshipRules';
import { assertImportModelSupported } from './import/ValidateModel';
...
BaseViewer.prototype.importXML = async function(xml, view, isNewModel) {
  ...
  try {
    ...
    if (model.languageVersion === '4.0') {
      const invalidRelationships = validateModelRelationships(model);
      if (invalidRelationships.length) {
        throw new Error(
          `Invalid ArchiMate 4 relationships: ${invalidRelationships.length}`
        );
      }
    }
    ...
    assertImportModelSupported(model);
...
BaseViewer.prototype.importModel = async function(model, view, elementsById) {
  ...
  const previousElementsById = this._elementsById;
  const previousRoot = this.get('canvas').getRootElement();
  const previousViewId = previousRoot && previousRoot.businessObject && previousRoot.businessObject.id;
  ...
  try {
    ...
    this._setModel(previousModel);
    this._setElementsById(previousElementsById);
    if (previousModel) {
      try {
        await this.openView(previousViewId);
      } catch (rollbackError) {
...
BaseViewer.prototype.saveSVG = function(options) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    try {
      ...
      if (!contentNode) {
        throw new Error('no active diagram layer');
      }
