...
NodeUpdater.prototype.createElementRef =  function(shape) {
  ...
  const view = shape.parent.businessObject;
  const viewElements = view.get('viewElements');
  if (!viewElements.find((element) => element.id === shape.businessObject.id)) {
    shape.businessObject.$parent = view;
    viewElements.push(shape.businessObject);
  }
