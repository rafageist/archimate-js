...
import { canConnect, normalizeAccessMode, normalizeAssociationMode } from '../../../archimate4/RelationshipRules';
import { getSemanticType } from '../../../archimate4/SemanticType';
...
ReplaceRelationshipRefHandler.prototype.execute = function(context) {
  ...
  const candidateType = newRelationshipRef && newRelationshipRef.type || connection.type;
  if (!canConnect(getSemanticType(source), candidateType, getSemanticType(target))) {
    throw new Error(`Relationship ${candidateType} is not allowed from ${getSemanticType(source)} to ${getSemanticType(target)}`);
  }
  ...
  if (connection.businessObject.type === CONNECTION_RELATIONSHIP) {
    ...
    if (!newRelationshipRef) {
      ...
      if (connection.type === RELATIONSHIP_ACCESS) {
        newRelationshipRef.accessType = normalizeAccessMode(connection.typeOption);
      } 
    
      if (connection.type === RELATIONSHIP_ASSOCIATION) {
        newRelationshipRef.isDirected = normalizeAssociationMode(connection.typeOption);
      }
    ...
    if (type === RELATIONSHIP_ACCESS) {
      connection.typeOption = normalizeAccessMode(newRelationshipRef.accessType);
    }

    if (type === RELATIONSHIP_ASSOCIATION) {
      connection.typeOption = normalizeAssociationMode(newRelationshipRef.isDirected);
    }
