...
import { getAllowedRelationships, canConnect as canConnectConcepts } from '../../archimate4/RelationshipRules';
...
function canConnect(source, target, connection) {
  ...
  if (source.businessObject.type === NODE_ELEMENT) { //is(source.businessObject, ARCHIMATE_NODE)) {
    if (target.businessObject.type === NODE_ELEMENT) { //is(target.businessObject, ARCHIMATE_NODE)) {
      const sourceType = getSemanticType(source);
      const targetType = getSemanticType(target);
      // connection.reconnect call
      if (connection) {
        const relationshipType = getSemanticType(connection);
        if (canConnectConcepts(sourceType, relationshipType, targetType)) {
          return { type: connection.type};
        } else {
          return false;
        }
      } else {
        return getAllowedRelationships(sourceType, targetType).length
          ? { type: CONNECTION_RELATIONSHIP }
          : false;
      }
    }
    if (is(target.businessObject, ARCHIMATE_CONNECTION)) return false;
    if (target.type === NOTE) {
      return { type: CONNECTION_LINE };
    }
  }
  if (is(source.businessObject, ARCHIMATE_CONNECTION)) {
    if (target.businessObject.type === NODE_ELEMENT) return false;
    if (target.type === NOTE) {
      return { type: CONNECTION_LINE };
    }
  }
