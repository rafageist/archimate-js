...
ConnectionMenuProvider.prototype.getEntries = function(element) {
  ...
  if (element.type !== CONNECTION_RELATIONSHIP) {
    ...
    relationshipsAllowed = getRelationshipsAllowed(getSemanticType(element.source), getSemanticType(element.target), getSemanticType(element));
  } else {
    relationshipsAllowed = getRelationshipsAllowed(getSemanticType(element.source), getSemanticType(element.target));
  }
  var reverseRelationshipsAllowed = getRelationshipsAllowed(getSemanticType(element.target), getSemanticType(element.source));
